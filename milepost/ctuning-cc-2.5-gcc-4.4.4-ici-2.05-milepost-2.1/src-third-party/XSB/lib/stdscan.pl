/* a general purpose scanner for standardizing -- from XSB */

/* -- needs some rewriting before we put it in the library.
   1) figure out how to parameterize digit, letter etc better.
   2) update modules, etc.
*/

:- use_module(stdscan,[scan/2, scan/3, set_scan_pars/1, get_scan_pars/1]).

%TLS: these module definitions need to be updated
:- use_module(usermod,[whitespace/1, special_char/1, digit/1, letter/1]).
