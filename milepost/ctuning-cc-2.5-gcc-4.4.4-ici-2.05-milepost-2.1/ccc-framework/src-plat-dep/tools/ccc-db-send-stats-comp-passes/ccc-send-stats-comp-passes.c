...
int main(int argc, char* argv[])
{
  ...
  /********************************************************************/
  printf("CCC SEND STATS COMP PASSES (send compilation statistics to SQL server)\n\n");

  if (argc<2)
  {
    printf("Usage:\n");
    printf(" ccc-send-stats-comp-passes <Local file with stats>\n");
  ...
  while (fgets(str1, 8191, ff)!=NULL)
  {
    ...
    if (separate_str(str1, str2, str3)>0)
    {
      ...
      if (strcmp(str2, stat_comp_COMPILER_ID)==0)
      {
        ignore=1;
        strcpy(var_compiler_id, str3);
        printf("DETECTED %s=%s\n", stat_comp_COMPILER_ID, var_compiler_id);
      }
      else if (strcmp(str2, stat_comp_DATETIME)==0)
      {
        ignore=1;
        strcpy(comp_datetime, str3);
      }
      ...
      else if (strcmp(str2, stat_comp_PASSES)==0)
      {
        /* Process OPT_PASSES_ID */
        ignore=1;
	...
	printf("  Processing %s\n", stat_comp_PASSES);
        ...
        sprintf(str4, "SELECT %s FROM %s WHERE (%s='%s' AND %s='%s')", 
	              stat_comp_OPT_PASSES_ID, ccc_opt_passes, 
                      stat_comp_COMPILER_ID, var_compiler_id,
		      stat_comp_PASSES, str3);
	...
	if (found==0)
	{
          ...
          /* Add Flag */
          printf("  Add passes sequence to the database!\n");

          sprintf(str4, "INSERT INTO %s (%s,%s,%s,%s,%s,%s) VALUES ('%s','%s','%s','%s','%s','%s')", 
	                 ccc_opt_passes, 
			 stat_comp_OPT_PASSES_ID, stat_comp_COMPILER_ID, stat_comp_PASSES, 
			 stat_comp_DATETIME, stat_comp_USER, stat_comp_IP,
			 str5, var_compiler_id, str3, comp_datetime, comp_user, comp_ip);
	  
          if (mysql_query(conn_c, str4))
          {
            printf("CCC Error: Can't store passes sequence in the database (%s)\n", mysql_error(conn_c));
            exit(1);
          }
 	  ...
 	  /* ADD found OPT_ID */
          ...
          printf("  New %s = %s\n", stat_comp_OPT_PASSES_ID, str5);
	}
	else
	{
          printf("  Found passes sequence in the database!\n");
	}
      }
  ...
  /* Prepare query */
  sprintf(str1, "INSERT INTO %s (%s) VALUES (%s)", ccc_stat_comp_passes, str_keys, str_values);
