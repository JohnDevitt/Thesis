...
int main(int argc, char* argv[])
{
  ...
  /********************************************************************/
  printf("CCC SEND STATS RUN (send compilation statistics to SQL server)\n\n");

  if (argc<2)
  {
    printf("Usage:\n");
    printf(" ccc-send-stats-run <Local file with stats>\n");
  ...
  while (fgets(str1, 8191, ff)!=NULL)
  {
    ...
    if (separate_str(str1, str2, str3)>0)
    {
      ...
      if (strcmp(str2, stat_run_PROGRAM_ID)==0)
      {
        printf("  %s detected\n", stat_run_PROGRAM_ID);
	strcpy(prog_id, str3);
      }
      else if (strcmp(str2, stat_run_RUN_COMMAND_LINE)==0)
      {
        /* Process RUN_COMMAND_LINE */

        int found=0;
	
	printf("  Processing %s\n", stat_run_RUN_COMMAND_LINE);

        /* Search exisiting datasets */
        sprintf(str4, "SELECT %s FROM %s WHERE (%s='%s' AND %s='%s')", 
	              stat_run_DATASET_ID, ccc_datasets, 
                      stat_run_PROGRAM_ID, prog_id,
		      stat_run_RUN_COMMAND_LINE, str3);
        if (mysql_query(conn_c, str4)) 
        {
          printf("CCC Error: Can't query database (%s)!\n", mysql_error(conn_c));
          exit(1);
	  ...
	  /* ADD found DATASET */
          ...
          strcat(str_keys, stat_run_DATASET_ID);
          strcat(str_values, "'");
          strcat(str_values, row[0]);
          strcat(str_values, "'");
	  
          strcpy(str3, ""); /* Clean the RUN_COMMAND_LINE key */	  
        }
	...
	{
          /* Add DATASET */
          printf("  Add dataset to the database!\n");
	  ...
	  /* Generate DATASET ID */
          ...
          sprintf(str4, "INSERT INTO %s (%s,%s,%s) VALUES ('%s','%s','%s')", 
	                 ccc_datasets, 
			 stat_run_DATASET_ID, stat_run_PROGRAM_ID, stat_run_RUN_COMMAND_LINE, 
			 str5, prog_id, str3);
	  
          if (mysql_query(conn_c, str4))
          {
            printf("CCC Error: Can't store datasets in the database (%s)\n", mysql_error(conn));
            exit(1);
          }

	  /* ADD found DATASET_ID */
          if (strlen(str_keys)!=0) strcat(str_keys, ",");
          if (strlen(str_values)!=0) strcat(str_values, ",");
          ...
          printf("  New %s = %s\n", stat_run_DATASET_ID, str5);
	}
	else
	{
          printf("  Found item in the database!\n");
	}
      }
  ...
  /* Prepare query */
  sprintf(str1, "INSERT INTO %s (%s) VALUES (%s)", ccc_stat_run, str_keys, str_values);
